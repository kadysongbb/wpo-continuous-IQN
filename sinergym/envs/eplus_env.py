...
class EplusEnv(gym.Env):
    ...
    def __init__(
        ...
        self.obs_dict = None
        self.energy_cost_avg = 0
        self.comfort_percent = 0
        self.info_dic = {'eplus_info': []}
    ...
    def step(self,
        ...
        ts = int(
                time_info[3] / self.simulator._eplus_run_stepsize)
        eps_length = 35040
        if (ts + 1) % (eps_length + 1) == 0:
            self.energy_cost_avg = self.energy_cost_avg / eps_length
            self.comfort_percent = self.comfort_percent / eps_length
            print('\n===================================================================')
            print('Average Energy Cost: ' + str(self.energy_cost_avg))
            print('Comfort Violation Percentage: ' + str(self.comfort_percent))
            print('===================================================================\n')
            self.info_dic['eplus_info'].append((ts, self.energy_cost_avg, self.comfort_percent))
            with open('eplus_log.txt', 'w') as file:
                file.write(json.dumps(self.info_dic))
            self.energy_cost_avg = 0 
            self.comfort_percent = 0
        self.energy_cost_avg += power
        if terms['reward_comfort'] != 0:
            self.comfort_percent += 1
