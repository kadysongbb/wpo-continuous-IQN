...
class GACAgent:
    ...
    def _sample_positive_advantage_actions(self, states):
        ...
        # reorder good states, actions, advantages
        new_indices = tf.nn.top_k(good_states[:, 0], k=len(good_states), sorted=False).indices
        reordered_good_states = tf.gather(good_states, new_indices)
        reordered_good_actions = tf.gather(good_actions, new_indices)
        reordered_advantages = tf.gather(advantages, new_indices)
        return reordered_good_states, reordered_good_actions, reordered_advantages
