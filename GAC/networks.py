...
class IQNActor(tf.Module):
    ...
    def target_density(self, mode, states, supervise_actions, advantage, beta):
        """
        ...
        if mode == "wpo":
            ...
            for i in range(len(states)):
                if states[i][0] != cur_state[0] or i == len(states) -1:
                    temp_weights = np.zeros(i-start_idx)
                    # move j to which k 
                    for j in range(start_idx, i):
                        values = np.zeros(i-start_idx)
                        for k in range(start_idx, i):
                            values[k-start_idx] = advantage[k] - beta*np.linalg.norm(supervise_actions[j]-supervise_actions[k])
                        k_best = np.argmax(values)
                        temp_weights[k_best] += 1
                    temp_weights = temp_weights/np.sum(temp_weights)
                    weights[start_idx:i] = temp_weights
                    cur_state = states[i]
                    start_idx = i
            weights = tf.convert_to_tensor(weights, dtype=np.float32)
            weights = tf.reshape(weights,[len(states),1])
            return weights
    ...
    def train(self, states, supervise_actions, advantage, mode, beta):
        ...
        weights = self.target_density(mode, states, supervise_actions, advantage, beta)
